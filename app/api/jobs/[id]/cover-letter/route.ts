...
export async function GET(
  ...
  try {
    ...
    const includeJobTitle = req.nextUrl.searchParams.get("title") !== "0";
    ...
    if (!includeResume || !tailoredContent) {
      return new NextResponse(new Uint8Array(clBuffer), {
        headers: {
          ...
          "Content-Disposition": `attachment; filename="${safeCompany}.pdf"`,
        },
      });
    }
    ...
    return new NextResponse(new Uint8Array(mergedBuffer), {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${safeCompany}-resume.pdf"`,
      },
    });
  } catch (err) {
...
// POST — download PDF using text from request body (unsaved edits)
export async function POST(
  ...
  try {
    ...
    const [jobRes, profileRes] = await Promise.all([
      insforge.database
        .from("jobs")
        .select("title, company, about_role, responsibilities, requirements, tailored_resume_content")
        .eq("id", jobId)
        .eq("user_id", userId)
        .single(),
      insforge.database
    ...
    const allJobText = [job.title ?? "", job.about_role ?? "", ...((job.responsibilities as string[] | null) ?? []), ...((job.requirements as string[] | null) ?? [])].join(" ");
    const lang = detectLanguage(allJobText || text);
    const labels = LANGUAGE_LABELS[lang] ?? LANGUAGE_LABELS.en;
    const avatarUrl = includePhoto && profile.avatar_url ? profile.avatar_url : null;
    const email = profile.email || authData.user.email || null;
    const contactParts: string[] = [];
    if (profile.phone) contactParts.push(profile.phone);
    if (email) contactParts.push(email);
    if (profile.portfolio_url) contactParts.push(profile.portfolio_url.replace(/^https?:\/\//, "").replace(/\/$/, ""));
    const safeCompany = job.company.toLowerCase().replace(/[^a-z0-9]+/g, "-");

    const clElement = createElement(CoverLetterPDF, { fullName: profile.full_name ?? "", jobTitle: job.title, company: job.company, coverLetterText: text, contactParts, avatarUrl, labels, includeJobTitle }) as unknown as ReactElement<DocumentProps>;
    const clBuffer = await renderToBuffer(clElement);

    const tailoredContent = job.tailored_resume_content as Record<string, unknown> | null;
    if (!includeResume || !tailoredContent) {
      return new NextResponse(new Uint8Array(clBuffer), {
        headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="${safeCompany}.pdf"` },
      });
    }

    const skillYears = computeSkillYears(profile.work_experience);
    const resumeElement = createElement(ResumePDF, { profile, generated: tailoredContent as Parameters<typeof ResumePDF>[0]["generated"], skillYears }) as unknown as ReactElement<DocumentProps>;
    const resumeBuffer = await renderToBuffer(resumeElement);
    const merged = await PDFDocument.create();
    for (const buf of [clBuffer, resumeBuffer]) {
      const src = await PDFDocument.load(buf);
    ...
    return new NextResponse(new Uint8Array(Buffer.from(await merged.save())), {
      headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="${safeCompany}-resume.pdf"` },
    });
  } catch (err) {
    Sentry.captureException(err);
    console.error("[api/jobs/cover-letter POST]", err);
    return NextResponse.json({ error: "Something went wrong. Please try again." }, { status: 500 });
  }
}
