...
export async function POST(req: NextRequest) {
  try {
    ...
    const slot = (formData.get("slot") as string) ?? "0";
    const projectId = (formData.get("projectId") as string) ?? "unknown";
    const path = `${authData.user.id}/project-${projectId}-${slot}.${ext}`;
    ...
    if (uploadError || !uploaded) return NextResponse.json({ error: "Upload failed." }, { status: 500 });

    return NextResponse.json({ url: uploaded.url });
  } catch (err) {
    console.error("[api/profile/project-image POST]", err);
